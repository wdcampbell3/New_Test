<script lang="ts">
	...
	function moveWorm() {
		...
		if (worm.some((segment) => segment.x === newHead.x && segment.y === newHead.y)) {
			endGame();
			return;
		}
	...
	<div class="flex flex-col items-center gap-6">
			...
			<div class="card-body p-4">
				...
				>
					{#each Array(GRID_SIZE * GRID_SIZE) as _, i}
						...
						{@const isWorm = worm.some((segment) => segment.x === x && segment.y === y)}
		...
		<div class="text-center">
			{#if !gameStarted && !gameOver}
				<button onclick={startGame} class="btn btn-primary btn-lg">Start Game</button>
			{/if}
