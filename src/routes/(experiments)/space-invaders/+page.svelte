<script lang="ts">
  ...
  // Bullets
  let bullets = $state<Array<{x: number, y: number, width: number, height: number}>>([]);
  const BULLET_SPEED = 8;

  // Aliens
  let aliens = $state<Array<{x: number, y: number, width: number, height: number, alive: boolean}>>([]);
  let alienDirection = $state(1);
  let alienSpeed = $state(1);
  let alienMoveDown = $state(false);

  // Alien bullets
  let alienBullets = $state<Array<{x: number, y: number, width: number, height: number}>>([]);
  ...
  function draw() {
    ...
    if (!gameRunning && !gameOver) {
      ...
      ctx.fillText('Click START to begin', CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + 20);
      animationId = requestAnimationFrame(draw);
      return;
    }
      ...
      <div class="card-body">
        ...
        <div class="space-y-4">
          ...
          <div class="flex gap-2">
            ...
            {:else if !gameOver}
              <button class="btn btn-warning" onclick={() => gameRunning = false}>
                Pause
              </button>
            {/if}
            {#if gameOver}
          ...
          <div>
            ...
            <ul class="list-disc list-inside space-y-1 text-sm">
              <li>Use <kbd class="kbd kbd-sm">←</kbd> and <kbd class="kbd kbd-sm">→</kbd> to move</li>
